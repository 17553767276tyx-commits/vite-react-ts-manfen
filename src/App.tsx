...
const parseTextToQuestions = (text: string, categoryId: string): Question[] => {
    ...
    if (currentQ && currentQ.content) {
      ...
      if (!currentQ.type) {
        if (currentOptions.length > 0) {
          ...
          const cleanAns = match
            ? match[0].replace(/[^A-Z]/gi, '').toUpperCase()
            : '';
  ...
  lines.forEach((line) => {
    if (/^(\d+[\.、\)\s\uff0e\u3001]|^\(\d+\))/.test(line)) {
      ...
      const multiOptionLineMatch = line.match(
        /([A-Z][\.、\)\s\uff0e\u3001]|\([A-Z]\))/g
      );
      if (
        multiOptionLineMatch &&
        multiOptionLineMatch.length > 1 &&
        /^([A-Z][\.、\)\s\uff0e\u3001]|\([A-Z]\))/.test(line)
      ) {
        ...
        parts.forEach((p) => {
          if (/^([A-Z][\.、\)\s\uff0e\u3001]|\([A-Z]\))/.test(p))
            currentOptions.push(p);
        });
      } else if (/^([A-Z][\.、\)\s\uff0e\u3001]|\([A-Z]\))/.test(line)) {
        currentOptions.push(line);
      } else if (/^(答案|Answer|Ans|参考答案|【答案】)[:：]?/.test(line)) {
        const ans = line
          .replace(/^(答案|Answer|Ans|参考答案|【答案】)[:：]?\s*/, '')
          .trim();
        currentQ.answer = ans;
        if (
          ['对', '错', '正确', '错误', 'T', 'F', '√', '×'].includes(ans) ||
          ans.toUpperCase() === 'TRUE' ||
          ans.toUpperCase() === 'FALSE'
        ) {
        ...
        else if (line.includes('问答') || line.includes('简答'))
          currentQ.type = 'essay';
...
const HighlightIndicator = ({ level }: { level: number }) => {
  ...
  return (
    <div
      className={`w-3 h-3 rounded-full ${colors[level]} mr-2 inline-block shadow-sm ring-2 ring-white`}
    ></div>
  );
};
...
const NoteEditor = ({
  ...
  const [selectedFolderId, setSelectedFolderId] = useState(
    initialContent?.folderId || folders[0]?.id || 'default'
  );
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 relative z-20">
      ...
      {isEditing && (
        <div className="flex items-center gap-2 p-2 px-4 bg-slate-50 dark:bg-slate-800 overflow-x-auto animate-in slide-in-from-top-2 fade-in">
          ...
          <button
            ...
            className="w-6 h-6 rounded-full bg-yellow-200 border border-slate-200"
          ></button>
          <button
            onClick={() => handleHighlight('#bbf7d0')}
            className="w-6 h-6 rounded-full bg-green-200 border border-slate-200"
          ></button>
          <button
            onClick={() => handleHighlight('#fbcfe8')}
            className="w-6 h-6 rounded-full bg-pink-200 border border-slate-200"
          ></button>
          <div className="w-px h-6 bg-slate-300 dark:bg-slate-600 mx-1"></div>
          <button
            onClick={() => fileInputRef.current?.click()}
            className="p-2 rounded hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 flex items-center"
          >
            <ImageIcon size={18} />
          </button>
          <button
      ...
      <div
        className={`flex-1 p-4 overflow-y-auto outline-none text-slate-700 dark:text-slate-300 text-lg leading-relaxed editor-content ${
          !isEditing ? 'read-only' : ''
        }`}
...
const QuestionCard = memo(
  ({
    ...
    const isBookmarked = userState.bookmarkFolders.some((f) =>
      f.questionIds.includes(q.id)
    );
    ...
    const getQuestionTypeStyle = (type: QuestionType) => {
      switch (type) {
        ...
        case 'judgment':
          return 'bg-purple-50 text-purple-600 border border-purple-100 dark:bg-purple-900/30 dark:text-purple-400 dark:border-purple-800';
        case 'fill':
          return 'bg-cyan-50 text-cyan-600 border border-cyan-100 dark:bg-cyan-900/30 dark:text-cyan-400 dark:border-cyan-800';
        case 'essay':
          return 'bg-rose-50 text-rose-600 border border-rose-100 dark:bg-rose-900/30 dark:text-rose-400 dark:border-rose-800';
        default:
    ...
    const renderOption = (opt: string, idx: number) => {
      ...
      const isSelected =
        q.type === 'multiple' ? userVal.includes(optKey) : userVal === optKey;

      let btnClass =
        'border-slate-100 bg-slate-50 text-slate-600 hover:bg-slate-100 dark:border-slate-800 dark:bg-slate-800/50 dark:text-slate-300 dark:hover:bg-slate-800';
      let iconClass =
        'bg-slate-200 text-slate-500 dark:bg-slate-700 dark:text-slate-400';

      if (isSelected) {
        btnClass =
          'border-indigo-100 bg-indigo-50 text-indigo-700 shadow-sm ring-1 ring-indigo-200 dark:bg-indigo-900/40 dark:text-indigo-300 dark:border-indigo-800 dark:ring-indigo-800';
      ...
      if ((!data.isExam && data.showAnswer) || data.examSubmitted) {
        ...
        if (isCorrect) {
          btnClass =
            'border-emerald-100 bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200 dark:bg-emerald-900/40 dark:text-emerald-300 dark:border-emerald-800 dark:ring-emerald-800';
          iconClass = 'bg-emerald-500 text-white dark:bg-emerald-600';
        } else if (isSelected) {
          btnClass =
            'border-rose-100 bg-rose-50 text-rose-700 ring-1 ring-rose-200 dark:bg-rose-900/40 dark:text-rose-300 dark:border-rose-800 dark:ring-rose-800';
      ...
      return (
        ...
        >
          <span
            className={`w-6 h-6 flex-shrink-0 flex items-center justify-center rounded-full text-xs font-bold mr-3 ${iconClass}`}
          >
            {optKey}
          </span>
          <span className="leading-snug">
            {opt.replace(/^[A-Z][\.,、\)\s\uff0e\u3001]|^\([A-Z]\)/, '').trim()}
          </span>
        </button>
      );
    };
    ...
    return (
      <div className="flex flex-col h-full relative z-10 p-4 pb-24 max-w-2xl mx-auto w-full">
        <div className="flex justify-between items-center mb-6">
          <button
            onClick={onExit}
            className="text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 flex items-center bg-white/80 dark:bg-black/40 backdrop-blur-sm p-2 rounded-full shadow-sm"
          >
            <ChevronLeft size={20} />{' '}
            <span className="text-sm font-medium ml-1">退出</span>
          </button>
          <div
            className={`text-slate-600 dark:text-slate-300 font-bold text-sm px-4 py-1.5 rounded-full shadow-sm ${cardBgClass}`}
          >
          ...
          <div className="flex gap-2 items-center">
            ...
            {!data.examSubmitted && data.isExam && (
              <button
                ...
                className="bg-indigo-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow-lg hover:bg-indigo-700 transition-all active:scale-95 mr-2 flex items-center"
              >
            ...
            <button
              ...
              className={`p-2 rounded-full transition-colors flex items-center justify-center shadow-sm ${
                highlightLevel > 0
                  ? 'bg-orange-100 text-orange-600 ring-1 ring-orange-200 dark:bg-orange-900/50 dark:text-orange-300'
            ...
            <button
              ...
              className="p-2 rounded-full bg-white/80 dark:bg-slate-800/80 shadow-sm hover:bg-white dark:hover:bg-slate-700 text-slate-400 dark:text-slate-500 transition-all"
            >
              <Star
                size={20}
                className={
        ...
        <div
          className={`flex-1 overflow-y-auto p-6 md:p-8 rounded-3xl border border-slate-100 dark:border-slate-800 mb-4 transition-all duration-300 ${cardBgClass}`}
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <span
                className={`text-[10px] px-2 py-1 rounded font-bold tracking-wider uppercase ${getQuestionTypeStyle(
                  q.type
                )}`}
              >
            ...
            {isWrong && (
              <div className="flex items-center">
                <span className="text-[10px] text-rose-500 bg-rose-50 dark:bg-rose-900/30 px-2 py-1 rounded mr-2">
                  错误次数: {wrongCount}
                </span>
                <button
          ...
          <div
            className={`text-slate-800 dark:text-slate-100 font-bold mb-8 leading-loose tracking-wide whitespace-pre-wrap ${contentSize}`}
          ...
          {q.type === 'judgment' && (
            <div className="flex gap-4 h-24 md:h-32">
              {['√', '×'].map((opt, idx) => {
                ...
                let btnClass =
                  'bg-slate-50 border-slate-200 text-slate-400 hover:bg-slate-100 dark:bg-slate-800 dark:border-slate-700 dark:text-slate-500 dark:hover:bg-slate-750';

                if (isSelected) {
                  btnClass =
                    opt === '√'
                      ? 'bg-emerald-50 border-emerald-200 text-emerald-600 dark:bg-emerald-900/40 dark:border-emerald-800 dark:text-emerald-400'
                      : 'bg-rose-50 border-rose-200 text-rose-600 dark:bg-rose-900/40 dark:border-rose-800 dark:text-rose-400';
                }

                if ((!data.isExam && data.showAnswer) || data.examSubmitted) {
                  const correctAnswerIsThis = isJudgmentCorrect(
                    opt === '√' ? '正确' : '错误',
                    q.answer
                  );
                  if (correctAnswerIsThis) {
                    btnClass =
                      'bg-emerald-100 border-emerald-300 text-emerald-700 ring-2 ring-emerald-200 dark:bg-emerald-900/60 dark:border-emerald-700 dark:text-emerald-300 dark:ring-emerald-800';
                  } else if (isSelected) {
                    btnClass =
                      'bg-rose-100 border-rose-300 text-rose-700 ring-2 ring-rose-200 dark:bg-rose-900/60 dark:border-rose-700 dark:text-rose-300 dark:ring-rose-800 opacity-60';
                  }
                }
          ...
          {(data.showAnswer || data.examSubmitted) && (
            <div className="mt-8 p-6 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700 animate-in fade-in slide-in-from-bottom-2">
              ...
              <div
                className={`text-slate-800 dark:text-slate-200 font-medium whitespace-pre-wrap ${optionSize}`}
              >
                {q.answer}
              </div>
              ...
              {data.examSubmitted && q.type !== 'essay' && (
                <div
                  className={`mt-2 text-sm py-2 px-3 rounded-lg inline-block ${
                    (data.answers[q.id] || '')
                      .replace(/\s/g, '')
                      .toUpperCase() ===
                    (q.answer || '').replace(/\s/g, '').toUpperCase()
        ...
        <div
          className={`fixed bottom-0 left-0 w-full p-4 flex justify-between items-center max-w-2xl mx-auto right-0 z-20 shadow-[0_-5px_20px_rgba(0,0,0,0.02)] dark:shadow-none safe-area-bottom border-t border-slate-100 dark:border-slate-800 ${cardBgClass}`}
        >
          <button
          ...
          {!data.isExam && (
            <div className="text-xs text-slate-400 dark:text-slate-500 font-medium">
              {data.showAnswer
                ? '已显示解析'
                : ['single', 'judgment'].includes(q.type)
                ? '点击选项显示结果'
                : '思考后点击查看解析'}
            </div>
          )}
              ...
              得分:{' '}
              {
                Object.values(data.answers).filter(
                  (a, i) =>
                    ...
                    (data.list[i].answer || '').replace(/\s/g, '').toUpperCase()
                ).length
              }{' '}
              / {data.list.length}
            </div>
...
export default function App() {
  const [activeTab, setActiveTab] = useState<
    'home' | 'study' | 'exam' | 'upload' | 'bookmarks' | 'notes'
  >('home');
  ...
  const [pendingQuestionIdForFolder, setPendingQuestionIdForFolder] = useState<
    string | null
  >(null);
  ...
  const [showRandomSetupModal, setShowRandomSetupModal] = useState<{
    categoryId: string;
    categoryName: string;
  } | null>(null);
    ...
    if (loadedData) {
      try {
        ...
        if (Array.isArray(newState.bookmarks)) {
          newState.bookmarkFolders = [
            {
              id: 'fav_default',
              name: '默认收藏',
              questionIds: newState.bookmarks,
            },
          ];
        ...
        if (
          !newState.bookmarkFolders ||
          newState.bookmarkFolders.length === 0
        ) {
          newState.bookmarkFolders = [
            { id: 'fav_default', name: '默认收藏', questionIds: [] },
          ];
        }
        if (!newState.randomProgress) newState.randomProgress = {};
        if (!newState.notes) newState.notes = [];
        if (!newState.noteFolders)
          newState.noteFolders = [{ id: 'default', name: '默认笔记本' }];
      ...
      setQuestions([
        {
          id: '1',
          content: '《红楼梦》的作者是谁？',
          options: ['A. 曹雪芹', 'B. 罗贯中', 'C. 施耐庵', 'D. 吴承恩'],
          answer: 'A',
          type: 'single',
          categoryId: 'default',
        },
        ...
        },
        {
          id: '3',
          content: '以下属于四大发明的有：',
          options: ['A. 造纸术', 'B. 指南针', 'C. 蒸汽机', 'D. 火药'],
          answer: 'ABD',
          type: 'multiple',
          categoryId: 'default',
        },
        {
          id: '4',
          content: 'One apple a day, keeps the doctor ____.',
          answer: 'away',
          type: 'fill',
          categoryId: 'default',
        },
    ...
    if (showDeleteConfirm.type === 'category') {
      ...
      setUserState((prev) => {
        ...
        const catQuestions = questions.filter(
          (q) => q.categoryId === catId && q.type === type
        );
        catQuestions.forEach((q) => {
          if (newWrongList[q.id]) delete newWrongList[q.id];
        });
  ...
  const handleImport = () => {
    ...
    if (importType === 'new') {
      ...
      finalCatName =
        categories.find((c) => c.id === finalCatId)?.name || '未知分类';
    }
  ...
  const startRandomWithFilter = (
    categoryId: string,
    categoryName: string,
    typeFilter: QuestionType | 'all'
  ) => {
    const allCatQuestions = questions.filter(
    ...
    if (available.length === 0) {
      if (window.confirm(`该类型题目已全部做完！\n是否重置进度重新开始？`)) {
        ...
        const nextBatch = filteredQuestions
    ...
    startSession(
      nextBatch,
      `${categoryName} - 随机模考 (剩余${available.length}题)`,
      'random',
      categoryId
    );
  ...
  const handleAnswerChange = useCallback(
    (qId: string, value: string, type: QuestionType) => {
      setCurrentModeData((prev) => {
        if (!prev) return null;
        let newAnswers = { ...prev.answers };
        if (type === 'multiple') {
          const currentVal = newAnswers[qId] || '';
          let newVal = currentVal.includes(value)
            ? currentVal.replace(value, '')
            : currentVal + value;
      ...
      setTimeout(() => {
        setUserState((s) => {
          ...
          if (prev.examType === 'random' && prev.categoryId) {
            ...
            newRandomProgress[prev.categoryId] = Array.from(
              new Set([...oldDone, ...answeredIds])
            );
          }
          ...
          return {
            ...
            examHistory: [
              ...s.examHistory,
              {
                ...
                mode: prev.examType === 'random' ? 'random' : 'sequential',
              },
            ],
          };
        });
  ...
  const handleHighlight = (qId: string) => {
    ...
    setUserState((prev) => ({
      ...prev,
      highlights: { ...prev.highlights, [qId]: nextLevel },
    }));
    if (nextLevel > 0) {
      const isBookmarked = userState.bookmarkFolders.some((f) =>
        f.questionIds.includes(qId)
      ...
      bookmarkFolders: prev.bookmarkFolders.map((f) =>
        f.id === folderId
          ? {
              ...f,
              questionIds: [
                ...new Set([...f.questionIds, pendingQuestionIdForFolder]),
              ],
            }
          : f
      ),
    ...
    setUserState((prev) => {
      ...
      if (editingNote === 'new') {
        ...
        newNotes = newNotes.map((n) =>
          n.id === editingNote.id
            ? { ...n, title, content, folderId, updatedAt: now }
            : n
        );
      }
  ...
  const createBookmarkFolder = () => {
    ...
    setUserState((prev) => ({
      ...prev,
      bookmarkFolders: [...prev.bookmarkFolders, newFolder],
    }));
    setNewFolderName('');
  ...
  const confirmDeleteCategory = () => {
    ...
    setQuestions((prevQuestions) =>
      prevQuestions.filter((q) => q.categoryId !== catId)
    );
    setCategories((prevCategories) =>
      prevCategories.filter((c) => c.id !== catId)
    );
    setUserState((prev) => {
      const newRandomProgress = { ...prev.randomProgress };
      delete newRandomProgress[catId];
      ...
      <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 w-full max-w-md shadow-2xl">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={() => setShowRandomSetupModal(null)}>
          ...
          <div className="grid grid-cols-2 gap-3 max-h-[40vh] overflow-y-auto">
            <button
              onClick={() =>
                startRandomWithFilter(categoryId, categoryName, 'all')
              }
            ...
            >
              <span>混合随机</span>
              <span className="text-xs font-normal opacity-70 bg-indigo-200 dark:bg-indigo-800 px-2 py-0.5 rounded-full">
                所有题型
              </span>
            </button>
            ...
            {typeConfig.map((config) => {
              ...
              return (
                <button
                  ...
                  onClick={() =>
                    count > 0 &&
                    startRandomWithFilter(categoryId, categoryName, config.type)
                  }
                  disabled={count === 0}
  ...
  const GroupingModal = () => {
    ...
    const availableTypes = (
      ['single', 'multiple', 'judgment', 'fill', 'essay'] as QuestionType[]
    ).filter((t) => list.some((q) => q.type === t));
      ...
      <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 w-full max-w-md shadow-2xl max-h-[80vh] flex flex-col">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={() => setShowGroupingModal(false)}>
              <X size={20} className="text-slate-400" />
            </button>
          </div>
          ...
          {availableTypes.length === 0 && (
            <div className="text-center text-slate-400 py-4">暂无题目</div>
          )}
        </div>
      </div>
    );
      ...
      <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 w-full max-w-md shadow-2xl max-h-[80vh] flex flex-col">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={() => setShowWrongGroupingModal(false)}>
              <X size={20} className="text-slate-400" />
            </button>
          </div>
          ...
          <div className="overflow-y-auto p-1 space-y-3">
            ...
            {byCategory.length === 0 ? (
              <div className="text-center text-slate-400 py-4 bg-slate-50 dark:bg-slate-800 rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
                暂无分类错题数据
              </div>
            ) : (
                ...
                {byCategory.map((cat) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex items-center">
                        <span className="text-xs text-slate-500 bg-slate-100 dark:bg-slate-700 dark:text-slate-400 px-2 py-1 rounded mr-2">
                          {cat.count} 题
                        </span>
                        <button
                          onClick={(e) => {
                ...
                {(['small', 'normal', 'large', 'xlarge'] as FontSize[]).map(
                  (size) => (
                    <button
                      key={size}
                      onClick={() =>
                        setUserState((p) => ({ ...p, fontSize: size }))
                      }
                      className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${
                        userState.fontSize === size
              ...
              {userState.bgImage ? (
                <div className="relative rounded-xl overflow-hidden h-32 border border-slate-200 dark:border-slate-700 group">
                  ...
                  <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      onClick={clearBg}
                      className="bg-rose-500 text-white px-3 py-1 rounded text-xs flex items-center"
                    >
  ...
  return (
    <div
      className={`${userState.theme} min-h-screen font-sans text-slate-600 dark:text-slate-300`}
    >
      <div
        className="fixed inset-0 z-0 bg-cover bg-center transition-all duration-500"
        style={{
          backgroundImage: userState.bgImage
            ? `url(${userState.bgImage})`
            : undefined,
      ...
      {userState.bgImage && (
        <div
          className={`fixed inset-0 z-0 ${
            userState.theme === 'dark' ? 'bg-black/60' : 'bg-white/40'
      ...
      <div className="relative z-10 flex flex-col h-screen max-w-md mx-auto overflow-hidden shadow-2xl">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
              ...
              <div className="space-y-2 mb-4 max-h-60 overflow-y-auto">
                {userState.bookmarkFolders.map((folder) => (
                  <button
                    ...
                    className="w-full p-4 rounded-xl border border-slate-100 dark:border-slate-800 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 hover:border-indigo-200 text-left flex items-center transition-all"
                  >
              ...
              <div className="flex gap-2 pt-4 border-t border-slate-100 dark:border-slate-800">
                <input
                  type="text"
                  placeholder="新建收藏夹"
                  className="flex-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm outline-none dark:text-slate-200"
                  value={newFolderName}
                  onChange={(e) => setNewFolderName(e.target.value)}
                />
                <button
                  onClick={() => {
                    ...
                    setUserState((p) => ({
                      ...p,
                      bookmarkFolders: [...p.bookmarkFolders, newFolder],
                    }));
                    setNewFolderName('');
                  }}
            ...
            <QuestionCard
              ...
              categoryName={
                categories.find(
                  (c) =>
                    c.id ===
                    currentModeData.list[currentModeData.index].categoryId
                )?.name
              }
              ...
              onSetShowAnswer={(show) =>
                setCurrentModeData((p) =>
                  p ? { ...p, showAnswer: show } : null
                )
              }
            />
          ) : (
            <>
                ...
                <div className="p-6 space-y-8 relative z-10">
                  ...
                  <div className="flex gap-3 mb-2">
                    ...
                    <button
                      ...
                      className="p-2.5 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md rounded-full border border-slate-100 dark:border-slate-800 shadow-sm text-slate-600 dark:text-slate-300"
                    >
                      <Settings size={20} />
                    </button>
                  </div>
                  ...
                  {showSearch && searchQuery && (
                    <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-md rounded-2xl border border-slate-100 dark:border-slate-800 shadow-lg p-4 absolute top-20 left-6 right-6 z-50 max-h-[60vh] overflow-y-auto">
                      ...
                      {questions.filter((q) => q.content.includes(searchQuery))
                        .length === 0 ? (
                        ...
                        <div className="space-y-2">
                          {questions
                            .filter((q) => q.content.includes(searchQuery))
                            .slice(0, 10)
                            .map((q) => (
                              <div
                                key={q.id}
                                onClick={() =>
                                  startSession([q], '搜索结果', 'study')
                                }
                          ...
                          {questions.filter((q) =>
                            ...
                            <div
                              className="text-center text-xs text-indigo-500 pt-2 cursor-pointer"
                              onClick={() =>
                                startSession(
                                  questions.filter((q) =>
                                    q.content.includes(searchQuery)
                  ...
                  <div className="text-center py-4">
                    <h1 className="text-3xl font-bold text-slate-800 dark:text-white tracking-tight mb-2 drop-shadow-sm">
                      满分上岸
                    </h1>
                  ...
                  <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md p-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm mb-6">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      <div
                        onClick={() =>
                          prepareGrouping(questions, '全库', 'study')
                        }
                        className="p-4 bg-indigo-50 dark:bg-indigo-900/30 rounded-xl text-center cursor-pointer hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors"
                      >
                        <span className="font-bold text-indigo-700 dark:text-indigo-300">
                          全库练习
                        </span>
                        <span className="text-xs text-indigo-400 dark:text-indigo-300 ml-2">
                          {questions.length}题
                        </span>
                      </div>
                      {categories.map((cat) => {
                        ...
                        return (
                          <div
                            ...
                            className="p-4 border border-slate-100 dark:border-slate-700 rounded-xl flex justify-between items-center cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50 bg-white dark:bg-slate-900"
                          >
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div
                      ...
                      className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md p-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm hover:border-amber-200 dark:hover:border-amber-700 cursor-pointer flex flex-col items-center justify-center text-center h-28"
                    >
                    ...
                    <div
                      ...
                      className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md p-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm hover:border-pink-200 dark:hover:border-pink-700 cursor-pointer flex flex-col items-center justify-center text-center h-28"
                    >
                    ...
                    <div
                      onClick={() => setShowWrongGroupingModal(true)}
                      className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md p-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm hover:border-rose-200 dark:hover:border-rose-700 cursor-pointer flex flex-col items-center justify-center text-center h-28"
                    >
              ...
              {activeTab === 'upload' && (
                <div className="p-6 h-full flex flex-col relative z-10">
                  <h2 className="text-slate-800 dark:text-slate-100 font-bold text-xl mb-6 flex items-center">
                    <Upload className="mr-3 text-blue-500" /> 导入新题库
                  </h2>
                  ...
                  <div className="flex-1 flex flex-col bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm mb-6">
                    ...
                    <div className="bg-indigo-50 dark:bg-indigo-900/30 p-3 rounded-lg mb-4 text-xs text-indigo-700 dark:text-indigo-300 border border-indigo-100 dark:border-indigo-800">
                      <div className="font-bold mb-1 flex items-center">
                        <Info size={14} className="mr-1" /> 格式排版指南
                      </div>
                      <div className="grid grid-cols-2 gap-2 opacity-80">
                        <div>
                          <span className="font-bold">选择题：</span>
                          <br />
                          1. 题目...
                          <br />
                          A. xx B. xx
                        ...
                        <div>
                          <span className="font-bold">判断题：</span>
                          <br />
                          2. 题目...
                          <br />
                          答案：对
                        ...
                        <div>
                          <span className="font-bold">多选题：</span>
                          <br />
                          3. [多选]...
                          <br />
                          A.. B..
                        ...
                        <div>
                          <span className="font-bold">填空题：</span>
                          <br />
                          4. [填空]...
                          <br />
                          答案：内容
                    ...
                    <div className="mt-3 flex justify-between items-center">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          reader.onload = (ev) => {
                            ...
                            if (importType === 'new' && !uploadCatName)
                              setUploadCatName(
                      ...
                      <button
                        onClick={() => fileInputRef.current?.click()}
                        className="text-xs bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 px-3 py-1 rounded text-slate-600 dark:text-slate-300 flex items-center transition-colors"
                      >
                  ...
                  <button
                    ...
                    className="w-full py-4 bg-slate-800 dark:bg-indigo-600 text-white font-bold rounded-xl shadow-lg hover:bg-slate-700 dark:hover:bg-indigo-700 active:scale-95 transition-all"
                  >
                  ...
                  <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-800 pb-20">
                    ...
                    <div className="space-y-2">
                      {categories.map((cat) => (
                        <div
                          key={cat.id}
                          className="flex justify-between p-3 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-lg items-center group"
                        >
                          <span className="text-slate-600 dark:text-slate-300 font-medium">
                            {cat.name}
                          </span>
                          <div className="flex items-center">
                            <span className="text-xs text-slate-300 dark:text-slate-600 mr-3">
                              {
                                questions.filter((q) => q.categoryId === cat.id)
                                  .length
                              }{' '}
                              题
                            </span>
              ...
              {activeTab === 'exam' && (
                <div className="p-6 relative z-10 w-full">
                  <h2 className="text-slate-800 dark:text-slate-100 font-bold text-xl mb-6 flex items-center">
                    <Zap className="mr-3 text-amber-500" /> 考试配置
                  </h2>
                  <div className="space-y-4 pb-20">
                    {categories.map((cat) => {
                      const doneCount = (userState.randomProgress[cat.id] || [])
                        .length;
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex gap-2">
                            <button
                              onClick={() =>
                                handleRandomExamStart(cat.id, cat.name)
                              }
                    ...
                    <button
                      onClick={() =>
                        startSession(questions, '全真模拟', 'random')
                      }
                      className="w-full p-5 mt-4 rounded-xl bg-indigo-600 text-white font-bold shadow-xl hover:bg-indigo-700 active:scale-95 transition-all"
                    >
                      全库随机大考 (30题)
                    </button>
                  </div>
              ...
              {activeTab === 'bookmarks' && (
                <div className="p-6 h-full relative z-10 w-full flex flex-col">
                  <div className="flex justify-between items-center mb-6">
                    <h2 className="text-slate-800 dark:text-slate-100 font-bold text-xl flex items-center">
                      <Star className="mr-3 text-pink-500" /> 我的收藏
                    </h2>
                    <button
                    ...
                    {userState.bookmarkFolders.map((folder) => (
                      <div
                        ...
                        onClick={() =>
                          startSession(
                            questions.filter((q) =>
                              folder.questionIds.includes(q.id)
                            ),
                            folder.name,
                            'study'
                          )
                        }
                        className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md cursor-pointer flex justify-between items-center group transition-all"
                      >
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-slate-400">
                              {folder.questionIds.length} 题
                            </div>
                          </div>
                        </div>
                        {folder.id !== 'fav_default' && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              initiateDeleteBookmarkFolder(
                                folder.id,
                                folder.name
                              );
                            }}
                            className="p-2 text-slate-300 hover:text-rose-500 dark:text-slate-600 dark:hover:text-rose-400"
                          >
                            <Trash2 size={16} />
                          </button>
                        )}
              ...
              {activeTab === 'notes' && (
                <div className="p-6 h-full relative z-10 w-full flex flex-col">
                  <h2 className="text-slate-800 dark:text-slate-100 font-bold text-xl mb-6 flex items-center">
                    <PenTool className="mr-3 text-emerald-500" /> 学习笔记
                  </h2>
                        ...
                        >
                          ...
                          {folder.id !== 'default' && (
                            <span
                              onClick={(e) => {
                                e.stopPropagation();
                                initiateDeleteNoteFolder(
                                  folder.id,
                                  folder.name
                                );
                              }}
                              className={`ml-2 p-0.5 rounded-full hover:bg-red-500/20 ${
                                activeNoteFolderId === folder.id
                  ...
                  <div className="flex-1 overflow-y-auto pb-20 grid gap-3 content-start">
                    {userState.notes.filter(
                      (n) =>
                        activeNoteFolderId === 'all' ||
                        n.folderId === activeNoteFolderId
                    ).length === 0 ? (
                      ...
                      userState.notes
                        ...
                        .map((note) => (
                          <div
                            ...
                            className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col group cursor-pointer hover:border-emerald-200 dark:hover:border-emerald-800 transition-colors"
                          >
                            <div className="flex justify-between items-start mb-2">
                              ...
                              <div className="flex items-center">
                                <span className="text-[10px] text-slate-400 mr-3">
                                  {new Date(
                                    note.updatedAt
                                  ).toLocaleDateString()}
                                </span>
        ...
        {!currentModeData && (
          <div className="h-20 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md border-t border-slate-100 dark:border-slate-800 relative z-20 flex justify-around items-center max-w-2xl mx-auto w-full pb-2 transition-colors">
            ...
            >
              <Upload size={24} />
              <span className="text-[10px] mt-1 font-medium">导入</span>
            </button>
            <button
            ...
            >
              <PenTool size={24} />
              <span className="text-[10px] mt-1 font-medium">笔记</span>
            </button>
            <button
              onClick={() => setActiveTab('bookmarks')}
              className={`flex flex-col items-center p-2 rounded-xl transition-all ${
            ...
            >
              <Star size={24} />
              <span className="text-[10px] mt-1 font-medium">收藏</span>
            </button>
          </div>
        )}
